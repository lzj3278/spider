...
############################
# File Name: info_sp.py
# Author: zhongjie.li
# email: zhongjie.li@viziner.cn
# Created Time: 2016-12-16 15:12:32
# Last Modified: 2016-12-16 17:40:31
############################
...
class ZhiHuClient(object):
    ...
    # TYPE_EMAIL = "email"
    loginURL = r"http://www.dowater.com/login/login1.asp"
    homeURL = r"http://www.dowater.com/"
    ...
    htmlFile = os.path.join(sys.path[0], "htmlfile")
    ...
    def login(self, username, password):
        ...
        # self.__loginURL = self.loginURL.format(self.__getUsernameType())o
        ...
        while True:
            ...
            data = {
                # "_xsrf": _xsrf,
                "UsernameGet": self.__username,
                "passwordget": self.__password,
                "imageField.x": 28,
                "imageField.y": 12,
            ...
            name_find_result = re.search(name_pat, content)
            print("=" * 50)
            # with open(self.htmlFile, 'wb') as f:
            # f.write(res.text)

            if name_find_result.group(1) == self.__username:
                print("登录成功")
                self.__saveCookie()
                break
            else:
...
if __name__ == '__main__':
    client = ZhiHuClient()
    client.login("15904262980", "04262980")
    ...
    # 用这个session进行其他网络操作，详见requests库
    session = client.getSession()
    url_nijian = "http://www.dowater.com/nijian/2016-12-16/516510.asp"
    index_info = session.get(url_nijian)
    # print(index_info.text.encode('gb2312').decode('utf-8'))

    print(index_info.content.decode('gb2312').encode('utf-8'))
